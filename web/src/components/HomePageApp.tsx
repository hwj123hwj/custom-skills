/**
 * HomePageApp - 首页 Island 组件
 *
 * 这是整个首页（技能市场）的客户端交互层。
 * 从原 App.tsx 的 HomePage 函数组件抽取，移除了 react-router 依赖。
 * 卡片点击改为打开 Modal（保留原体验），详情页链接用 <a> 标签跳转。
 */
  ...
  return (
    <>
        ...
        <>
            ...
            <>
              <div className="max-w-4xl mx-auto mb-6 sm:mb-8">
                <div className="flex flex-wrap gap-2 sm:gap-3 justify-center items-center">
                  <CategoryChip label={t('skill.category.all')} count={skillCategoryCounts['all']} active={activeSkillCategory === 'all' && !showFavorites} onClick={() => { setActiveSkillCategory('all'); setShowFavorites(false); }} />
                  <CategoryChip label={t('skill.category.design')} count={skillCategoryCounts['design']} active={activeSkillCategory === 'design' && !showFavorites} onClick={() => { setActiveSkillCategory('design'); setShowFavorites(false); }} />
                  <CategoryChip label={t('skill.category.coding')} count={skillCategoryCounts['coding']} active={activeSkillCategory === 'coding' && !showFavorites} onClick={() => { setActiveSkillCategory('coding'); setShowFavorites(false); }} />
                  <CategoryChip label={t('skill.category.content')} count={skillCategoryCounts['content']} active={activeSkillCategory === 'content' && !showFavorites} onClick={() => { setActiveSkillCategory('content'); setShowFavorites(false); }} />
                  <CategoryChip label={t('skill.category.platform')} count={skillCategoryCounts['platform']} active={activeSkillCategory === 'platform' && !showFavorites} onClick={() => { setActiveSkillCategory('platform'); setShowFavorites(false); }} />
                  <CategoryChip label={t('skill.category.knowledge')} count={skillCategoryCounts['knowledge']} active={activeSkillCategory === 'knowledge' && !showFavorites} onClick={() => { setActiveSkillCategory('knowledge'); setShowFavorites(false); }} />
                  <CategoryChip label={t('skill.category.product')} count={skillCategoryCounts['product']} active={activeSkillCategory === 'product' && !showFavorites} onClick={() => { setActiveSkillCategory('product'); setShowFavorites(false); }} />
                  <CategoryChip label={t('skill.category.engineering')} count={skillCategoryCounts['engineering']} active={activeSkillCategory === 'engineering' && !showFavorites} onClick={() => { setActiveSkillCategory('engineering'); setShowFavorites(false); }} />
                  <FavoritesBar favoriteCount={favoriteSkillCount} showFavorites={showFavorites} onToggleFavorites={() => { setShowFavorites(!showFavorites); setActiveSkillCategory('all'); }} />
                </div>
              </div>
          ...
          {activeTab === 'prompts' && (
            <>
              <div className="max-w-3xl mx-auto mb-6 sm:mb-8">
                <div className="rounded-2xl p-4 sm:p-5" style={{ background: 'var(--bg-card)', border: '1px solid var(--border-default)' }}>
                  <h3 className="text-sm font-semibold mb-2" style={{ color: 'var(--text-primary)' }}>{t('prompt.banner_title', { defaultValue: '提示词库' })}</h3>
                  <p className="text-sm leading-relaxed" style={{ color: 'var(--text-secondary)' }}>{t('prompt.banner_description', { defaultValue: '精心收集的高质量提示词，一键复制即可使用。' })}</p>
                </div>
              </div>
              <div className="max-w-4xl mx-auto mb-6 sm:mb-8 flex justify-center">
                <FavoritesBar favoriteCount={prompts.filter((p) => isFavorite(p.id)).length} showFavorites={showFavorites} onToggleFavorites={() => setShowFavorites(!showFavorites)} />
              </div>
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6 justify-items-center">
                {filteredPrompts.map((prompt) => (
                  <PromptCard key={prompt.id} prompt={prompt} onClick={handlePromptClick} isFavorite={isFavorite(prompt.id)} onToggleFavorite={toggleFavorite} />
                ))}
              </div>
              {filteredPrompts.length === 0 && (
                <div className="text-center py-20">
                  <p className="text-lg" style={{ color: 'var(--text-muted)' }}>
                    {showFavorites ? t('favorites.empty_prompts', { defaultValue: 'No favorite prompts yet.' }) : t('search.no_results_prompts', { defaultValue: 'No prompts found', query: searchQuery })}
                  </p>
                  {!showFavorites && (<button onClick={() => setSearchQuery('')} className="mt-4 font-medium transition-colors" style={{ color: 'var(--accent)' }}>{t('search.clear')}</button>)}
                </div>
              )}
            ...
            <>
              <div className="max-w-4xl mx-auto mb-6 sm:mb-8">
                <div className="flex flex-wrap gap-2 sm:gap-3 justify-center">
                  <CategoryChip label={t('deck.category.all')} count={decks.length} active={activeDeckCategory === 'all'} onClick={() => setActiveDeckCategory('all')} colorScheme="amber" />
                  <CategoryChip label={t('deck.category.knowledge_cards')} count={deckCategoryCounts['knowledge-cards']} active={activeDeckCategory === 'knowledge-cards'} onClick={() => setActiveDeckCategory('knowledge-cards')} colorScheme="amber" />
                  <CategoryChip label={t('deck.category.decision_decks')} count={deckCategoryCounts['decision-decks']} active={activeDeckCategory === 'decision-decks'} onClick={() => setActiveDeckCategory('decision-decks')} colorScheme="amber" />
                  <CategoryChip label={t('deck.category.workflow_notes')} count={deckCategoryCounts['workflow-notes']} active={activeDeckCategory === 'workflow-notes'} onClick={() => setActiveDeckCategory('workflow-notes')} colorScheme="amber" />
                </div>
              </div>
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6 justify-items-center">
              ...
              {filteredDecks.length === 0 && (
                <div className="text-center py-20">
                  <p className="text-lg" style={{ color: 'var(--text-muted)' }}>{t('search.no_results_decks', { query: searchQuery })}</p>
                  <button onClick={() => setSearchQuery('')} className="mt-4 font-medium transition-colors" style={{ color: 'var(--accent)' }}>{t('search.clear')}</button>
                </div>
              )}
            </>
      ...
      <DeckModal
        ...
        onViewDetail={selectedDeck ? () => navigateTo(`/deck/${selectedDeck.id}`) : undefined}
      />
      <PromptModal prompt={selectedPrompt} isOpen={!!selectedPrompt} onClose={() => setSelectedPrompt(null)} />
    </>
  );
}
