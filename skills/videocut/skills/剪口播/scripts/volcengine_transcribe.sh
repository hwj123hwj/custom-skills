...
# 获取 API Key — 从脚本所在位置向上遍历查找 .env
SCRIPT_DIR="$(cd "$(dirname "$0")" && pwd)"
ENV_FILE=""
_d="$SCRIPT_DIR"
for _ in 1 2 3 4 5 6 7 8; do
  if [ -f "$_d/.env" ] && grep -q VOLCENGINE_API_KEY "$_d/.env" 2>/dev/null; then
    ENV_FILE="$_d/.env"
    break
  fi
  _d="$(dirname "$_d")"
...
if [ -z "$ENV_FILE" ]; then
  echo "❌ 找不到包含 VOLCENGINE_API_KEY 的 .env 文件"
  echo "从 $SCRIPT_DIR 向上查找了 8 层均未找到"
  echo "请在项目目录创建 .env 并填入 VOLCENGINE_API_KEY=xxx"
  exit 1
fi
