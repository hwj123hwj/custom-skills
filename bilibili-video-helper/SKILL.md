...
大部分 API 需要登录 Cookie 才能正常访问。引导用户提供 Cookie：
...
#### 步骤 1: 获取 AID
```bash
AID=$(curl -s "https://api.bilibili.com/x/web-interface/view?bvid=$BVID" -H "Cookie: $COOKIE" | jq -r '.data.aid')
```
...
- **Cookie 必需**：所有 API 均需要用户提供登录 Cookie。
